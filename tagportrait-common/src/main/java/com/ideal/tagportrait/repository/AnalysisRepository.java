...
public interface AnalysisRepository extends JpaRepository<Analysis, Long> {
    ...
    @Query(value = "SELECT a.tag_num,t.name FROM tb_analysis a,tb_tag t,tb_tag c WHERE t.id = a.tag_id AND a.tag_id IN (1221,7,45,1035,1247,1257) AND a.area_id=?1 AND t.parent_id=0 AND c.id=t.id ORDER BY t.id ", nativeQuery = true)
